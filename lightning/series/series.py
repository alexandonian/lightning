...
class Series(Data):
    ...
    def tolocal(self):
        ...
        """
        from lightning.series.readers import fromarray
    ...
    def toimage(self, size='150'):
        """
        Converts Series to Image.

        Equivalent to calling series.toBlocks(size).toImage()
        ...
        if self.mode == 'spark':
            return Image(self.values.swap(tuple(range(n)), (0,), size=size))

        if self.mode == 'local':
            return Image(self.values.transpose((n,) + tuple(range(0, n))))
    ...
    def tobinary(self, path, prefix='series', overwrite=False, credentials=None):
        ...
        """
        from lightning.series.writers import tobinary
