...
class Image(Data):
    ...
    def __init__(self, values, labels=None, mode='local'):
        super(Image, self).__init__(values, mode=mode)
        self.labels = labels
    ...
    def count(self):
        """
        Count the number of channels in the image.
    ...
    def toblocks(self, chunk_size='auto', padding=None):
        """
        Convert to blocks which represent subdivisions of the image data
        (i.e. image channels).
        ...
        """
        from lightning.blocks.blocks import Blocks
        from lightning.blocks.local import LocalChunks
    ...
    def toseries(self, chunk_size='auto'):
        ...
        This method is equivalent to image.toblocks(size).toSeries().
        ...
        """
        from lightning.series.series import Series
    ...
    def tolocal(self):
        ...
        if self.mode == 'local':
            logging.getLogger('lightning').warn('image already in local mode')
            pass
    ...
    def tospark(self, engine=None):
        ...
        if self.mode == 'spark':
            logging.getLogger('lightning').warn('image already in spark mode')
            pass
    ...
    def sample(self, nsamples=100, seed=None):
        """
        Extract a random sample of image.
    ...
    def max_projection(self, axis=2):
        """
        Compute maximum projections of an image along a dimension.
    ...
    def gaussian_filter(self, sigma=2, order=0):
        """
        Spatially smooth an image with a gaussian filter.
    ...
    def uniform_filter(self, size=2):
        """
        Spatially filter an image using a uniform filter.
    ...
    def median_filter(self, size=2):
        """
        Spatially filter an image using a median filter.
    ...
    def _image_filter(self, filter=None, size=2):
        """
        Generic function for mapping a filtering operation over an image.
    ...
    def localcorr(self, size=2):
        ...
        from lightning.image.readers import fromarray, fromrdd
    ...
    def topng(self, path, prefix='image', overwrite=False):
        ...
        """
        from lightning.image.writers import topng
    ...
    def totif(self, path, prefix='image', overwrite=False):
        ...
        overwrite : bool
            If true, the directory given by path will first be deleted if it exists.
        """
        from lightning.image.writers import totif
        # TODO add back colormap and vmin/vmax
    ...
    def tobinary(self, path, prefix='image', overwrite=False):
        ...
        overwrite : bool
            If true, the directory given by path will first be deleted if it exists.
        """
        from lightning.image.writers import tobinary
    ...
    def map_as_series(self, func, value_size=None, dtype=None, chunk_size='auto'):
        """
        Efficiently apply a function to image as series data.

        For image data that represent image sequences, this method
        applies a function to each pixel's series, and then returns to
        the image format, using an efficient intermediate block
        representation.
        ...
        return blocks.map(f, value_shape=dims, dtype=dtype).toimage()
