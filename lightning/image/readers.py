...
def fromrdd(rdd, dims=None, nrecords=None, dtype=None, labels=None, ordered=False):
    ...
    values = BoltArraySpark(rdd.map(process_keys), shape=(nrecords,) + tuple(dims), dtype=dtype, split=1,
                            ordered=ordered)
    return Image(values, labels=labels)
...
def fromarray(values, labels=None, npartitions=None, engine=None):
    ...
    First dimension will be used to index image channels,
    so remaining dimensions after the first should
    be the dimensions of the image,
    e.g. (3, 100, 200) for 3 x (100, 200) channels

    Parameters
    ----------
    values : array-like
        The array of image. Can be a numpy array,
    ...
    if values.ndim < 2:
        raise ValueError('Array for image must have at least 2 dimensions, got %g' % values.ndim)
...
def fromlist(items, accessor=None, keys=None, dims=None, dtype=None, labels=None, npartitions=None, engine=None):
    """
    Load image from a list of items using the given accessor.
...
def frompath(path, accessor=None, ext=None, start=None, stop=None, recursive=False, npartitions=None, dims=None,
    ...
    """
    Load image from a path using the given accessor.
    ...
    """
    from lightning.readers import get_parallel_reader
...
def frompathlist(pathlist, accessor=None, keys=None, ext=None, start=None, stop=None, recursive=False, npartitions=None,
                 dims=None,
                 dtype=None, labels=None, recount=False, engine=None, credentials=None):
    from lightning.readers import get_parallel_reader

    if spark and isinstance(engine, spark):
        nrecords = len(pathlist)
        if keys:
            pathlist = zip(keys, pathlist)
        else:
            keys = [(i,) for i in range(nrecords)]
            pathlist = zip(keys, pathlist)
        if not npartitions:
            npartitions = engine.defaultParallelism
        rdd = engine.parallelize(pathlist, npartitions)
        if accessor:
            rdd = rdd.mapValues(a)
    else:
        values = asarray(
            [get_parallel_reader(path)(engine, credentials=credentials).read(path, ext=ext, start=start, stop=stop,
                                                                             recursive=recursive,
                                                                             npartitions=npartitions) for path in
             pathlist])
        return fromarray(values, labels=labels)
...
def frombinary(path, shape=None, dtype=None, ext='bin', start=None, stop=None, recursive=False, nplanes=None,
    ...
    """
    Load image from flat binary files.
    ...
    nplanes : positive integer, optional, default = None
        If passed, will cause single files to be subdivided into nplanes separate image.
...
def fromtif(path, ext='tif', start=None, stop=None, recursive=False, nplanes=None, npartitions=None, labels=None,
    ...
    """
    Loads image from single or multi-page TIF files.
        ...
        if nplanes is not None:
            ...
            if extra:
                if discard_extra:
                    ...
                    logging.getLogger('lightning').warn('Ignored %d pages in file %s' % (extra, fname))
                else:
...
def fromtiflist(tiflist, ext='tif', start=None, stop=None, recursive=False, nplanes=None, npartitions=None, labels=None,
                engine=None, credentials=None, discard_extra=False, keys=None, dims=None, dtype=None):
    import skimage.external.tifffile as tifffile
    return fromlist(tiflist, accessor=tifffile.imread, keys=keys, dims=dims, dtype=dtype, labels=labels, npartitions=npartitions,
             engine=engine)
