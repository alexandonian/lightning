...
def test_to_binary_roundtrip(tmpdir, eng):
    ...
    data = fromlist(a, engine=eng)
    data.tobinary(os.path.join(str(tmpdir), 'image'))
    loaded = frombinary(os.path.join(str(tmpdir), 'image'), engine=eng)
...
def test_to_png(tmpdir, eng):
    a = [arange(8, dtype='int16').reshape((4, 2)), arange(8, 16, dtype='int16').reshape((4, 2))]
    fromlist(a, engine=eng).topng(os.path.join(str(tmpdir), 'image'), prefix='image')
    files = [os.path.basename(f) for f in glob.glob(str(tmpdir) + '/image/image*')]
...
def test_to_png_roundtrip(tmpdir, eng):
    ...
    data = fromlist(a, engine=eng)
    data.topng(os.path.join(str(tmpdir), 'image'), prefix='image')
    loaded = frompng(os.path.join(str(tmpdir), 'image'))
    assert allclose(data.toarray(), loaded.toarray())


def test_to_tif(tmpdir, eng):
    a = [arange(8, dtype='int16').reshape((4, 2)), arange(8, 16, dtype='int16').reshape((4, 2))]
    fromlist(a, engine=eng).totif(os.path.join(str(tmpdir), 'image'), prefix='image')
    files = [os.path.basename(f) for f in glob.glob(str(tmpdir) + '/image/image*')]
...
def test_to_tif_roundtrip(tmpdir, eng):
    a = [arange(8, dtype='uint8').reshape((4, 2))]
    data = fromlist(a, engine=eng)
    data.totif(os.path.join(str(tmpdir), 'image'), prefix='image')
    loaded = fromtif(os.path.join(str(tmpdir), 'image'))
    assert allclose(data.toarray(), loaded.toarray())
