...
from lightning.image.readers import fromlist, fromarray
...
def test_toseries_roundtrip(eng):
    data = fromlist([arange(6).reshape((2, 3)), arange(6).reshape((2, 3))], engine=eng)
    assert isinstance(data.toseries(), Series)
    assert isinstance(data.toseries().toimage(), Image)
    assert allclose(data.toseries().toimage().toarray(), data.toarray())
