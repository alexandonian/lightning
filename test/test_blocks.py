...
from lightning.image.readers import fromlist
...
def test_roundtrip(eng):
    a = arange(8).reshape((4, 2))
    data = fromlist([a, a], engine=eng)
    vals = data.toblocks((2, 2)).toimage()
...
def test_map(eng):
    a = arange(8).reshape((4, 2))
    data = fromlist([a, a], engine=eng)
    map1 = data.toblocks((4, 2)).map(lambda x: 1.0 * x, dtype=float64).toimage()
    map2 = data.toblocks((4, 2)).map(lambda x: 1.0 * x).toimage()
